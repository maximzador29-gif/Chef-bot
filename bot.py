...
SECTIONS = [
    {"icon": "🌅", "title": "ОТКРЫТИЕ СМЕНЫ", "items": [
    ...
    {"icon": "🧑‍🍳", "title": "УПРАВЛЕНИЕ КОМАНДОЙ", "items": [
    ...
    {"icon": "🥩", "title": "ЗАГОТОВКИ И ПРОДУКТЫ", "items": [
    ...
    {"icon": "🛡️", "title": "КАЧЕСТВО И БЕЗОПАСНОСТЬ", "items": [
    ...
    {"icon": "🧹", "title": "ЧИСТОТА И ПОРЯДОК", "items": [
    ...
    {"icon": "🌙", "title": "ЗАКРЫТИЕ СМЕНЫ", "items": [
...
def build_report(state):
    ...
    lines = ["🍔 *ЧЕК-ЛИСТ СТАРШЕГО ПОВАРА*", f"👤 {state['name']} · {state['shift']} смена", f"📅 {date_str}", f"✅ Выполнено: {done}/{total} ({pct}%)", "—" * 20, ""]
    ...
    if data.startswith("toggle:"):
        ...
        if MANAGER_CHAT_ID:
            try:
                ...
                logger.error(f"Ошибка отправки руководителю: {e}")
    return CHECKLIST
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={ASK_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, ask_name)], ASK_SHIFT: [CallbackQueryHandler(ask_shift, pattern="^shift:")], CHECKLIST: [CallbackQueryHandler(handle_toggle)]},
